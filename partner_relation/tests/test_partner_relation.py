...
class TestPartnerRelation(TransactionCase):
    ...
    def test_relation(self):
        ...
        friend_rels = self.rpr.search([("relation_type_id", "=", self.friend_of.id)])
        self.assertEqual(len(friend_rels), 2)
        self.assertEqual(
            friend_rels[0].src_partner_id.id, friend_rels[1].dest_partner_id.id
        )
        self.assertEqual(
            friend_rels[1].src_partner_id.id, friend_rels[0].dest_partner_id.id
        )
        parent_rels = self.rpr.search([("relation_type_id", "=", self.parent_of.id)])
        self.assertEqual(len(parent_rels), 1)
        child_rels = self.rpr.search([("relation_type_id", "=", self.child_of.id)])
        self.assertEqual(len(child_rels), 1)
        self.assertEqual(
            child_rels[0].src_partner_id, self.env.ref("base.res_partner_address_13")
        )
        self.assertEqual(
            child_rels[0].dest_partner_id, self.env.ref("base.res_partner_address_11")
        )
    ...
    def test_write(self):
        ...
        self.assertEqual(
            friend1_rel.relation_type_id, self.env.ref("partner_relation.recommends")
        )
        self.assertEqual(
            friend1_rel.src_partner_id, self.env.ref("base.res_partner_address_1")
        )
        self.assertEqual(
            friend1_rel.dest_partner_id, self.env.ref("base.res_partner_address_15")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            friend1_inv_rel.src_partner_id, self.env.ref("base.res_partner_address_15")
        )
        self.assertEqual(
            friend1_inv_rel.dest_partner_id, self.env.ref("base.res_partner_address_1")
        )
